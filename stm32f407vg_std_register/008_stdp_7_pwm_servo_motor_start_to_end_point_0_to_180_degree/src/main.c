...
void gpio_conf(){
	...
	//timer PD13, pinsource13 dür.
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource13,GPIO_AF_TIM4); //alternate function olarak ayarladýk timer pwm için,

	init0.GPIO_Mode=GPIO_Mode_AF;
	init0.GPIO_OType=GPIO_OType_PP;
	init0.GPIO_Pin=GPIO_Pin_13;
...
int main(){
  ...
  while (1){
	  ...
	  for(i=500;i<=2500;i++){
		  ...
		  delay_ms(2); // 2ms de bir pulse degerini girecek.
	  }
	  for(i=2500;i>=500;i--){
		  init3.TIM_Pulse=i;
		  TIM_OC2Init(TIM4,&init3);
