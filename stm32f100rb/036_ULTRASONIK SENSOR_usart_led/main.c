...
// char by char olarak gelir ama while ile komple string bitene kadar gönderir böylelikle  kelime cümle seklinde gider
void uartTransmit(char *string){
   while(*string){
      while(!(USART1->SR & 0x00000040));
      USART_SendData(USART1,*string);
      string++; // pointer adress increment ,not content.
...
int main(){
	...
	while(1){
		...
		sprintf(message,"Mesafe: %f cm \n",distance); // distance variable 'ini char array'a dönüstürür.
